"""
Generates the Golden Evaluation Set of 200 hand-labeled test cases across 6 intents and edge cases.
"""
import json
import os
...
# 1. ORDER_STATUS_DELIVERY (33 cases)
order_cases = [
    ...
    ("Delivery van passed by my house without stopping even though app said next stop.", False),
    ("How do I add access code for my apartment building gate for the driver?", False),
    ("Order #114-001122 shows delivered but picture attached in app is someone else's porch!", False),
    ("Can I pick up my parcel directly from the local carrier sorting facility?", False),
    ("Is it possible to reschedule delivery date to next Monday?", False),
    ("Package tracking shows 'Held at Customs' for international shipping.", False),
    ("Driver marked 'Customer unavailable' but I was sitting on my front porch all day!", False),
    ("Where is my package #405-998877? Tracking link gives 404 error page.", False)
]

for idx, (text, esc) in enumerate(order_cases, 1):
    golden_cases.append({
        "id": idx,
...
return_cases = [
    ...
    ("Can I return an opened electronic item if it's missing original plastic wrap?", False),
    ...
    ("Item arrived damaged, do I still need to ship it back to get a refund?", False),
    ("Can I return a gift without the buyer knowing I returned it?", False),
    ("How do I return a bulky furniture item that doesn't fit in my car?", False),
    ("I dropped off return at UPS drop box without scanning label. Will it be processed?", False),
    ("Why is my return taking longer to process than my previous order?", False),
    ("Returned order #111-998877 10 days ago. Support chat promised refund in 48h.", False),
    ("What happens if my return package is lost by UPS in transit?", False),
    ("Can I return hazardous materials like lithium batteries by mail?", False)
]

for idx, (text, esc) in enumerate(return_cases, 34):
    golden_cases.append({
        "id": idx,
        "text": f"@AmazonHelp {text}",
...
# 3. PRODUCT_ISSUE_DEFECT (33 cases)
defect_cases = [
    ...
    ("The smartphone box was empty! Seal was broken and no phone inside!", True),
    ...
    ("The battery charger melted while plugged in! Potential fire hazard!", True),
    ("Expiration date on the food item delivered today was 3 months ago!", False),
    ("The power bank expanded and swollen! Looks dangerous to use.", True),
    ...
    ("The toy arrived with sharp exposed wire inside. Dangerous for kids!", True),
    ...
    ("Electric kettle sparked when plugged in and blew my home circuit breaker!", True),
    ("Received counterfeit perfume that smells like alcohol. Not genuine product!", False),
    ("Screen protector was cracked inside its sealed protective case.", False),
    ("Bicycle arrived missing front wheel and assembly hardware.", False),
    ("Food protein powder tub was unsealed and powder spilled inside box.", False),
    ("Wireless mouse sensor skips constantly and disconnects every 5 seconds.", False),
    ("Child car seat lock mechanism is jammed and won't buckle safely!", True),
    ("Memory card shows 32GB capacity when product description said 256GB.", False)
]

for idx, (text, esc) in enumerate(defect_cases, 67):
    golden_cases.append({
        "id": idx,
        "text": f"@AmazonHelp {text}",
...
# 4. ACCOUNT_DIGITAL_PRIME (34 cases)
account_cases = [
    ...
    ("My Amazon account was locked due to suspicious activity. Need access back!", True),
    ("Someone changed the email address on my Amazon account without my permission!", True),
    ...
    ("Account compromised! Someone placed 5 orders on my account in 10 minutes!", True),
    ...
    ("Cannot rental HD movie on Prime Video, error code 7031.", False),
    ("How to merge two separate Amazon accounts owned by same person?", False),
    ("Fire Tablet child profile PIN forgotten. How to reset?", False),
    ("Prime Reading books are not opening on desktop Cloud Reader.", False),
    ("Is Amazon Prime Gaming free loot still included in monthly subscription?", False),
    ("My account was suspended for reviewing products! I demand explanation!", True),
    ("How do I update payment card for Prime Video recurring billing?", False),
    ("Alexa app says device offline even though Echo Dot is connected to Wi-Fi.", False),
    ("Kindle Unlimited subscription renewed automatically after trial ended.", False)
]

for idx, (text, esc) in enumerate(account_cases, 100):
    golden_cases.append({
        "id": idx,
        "text": f"@AmazonHelp {text}",
...
# 5. PAYMENT_BILLING (33 cases)
billing_cases = [
    ("I was charged $14.99 twice on my bank statement for Prime this month!", True),
    ("There is a mysterious charge of $89.99 from Amazon Digital on my card.", True),
    ...
    ("Fraud alert: my credit card was charged for orders I never made!", True),
    ("Will take legal action if double charge of $350 is not refunded immediately!", True),
    ...
    ("I demand a full refund and compensation for unauthorized billing!", True),
    ...
    ("Double charge of $120.45 on my card statement from Amazon EU.", True),
    ("Payment processor rejected my international Visa debit card.", True),
    ("Charged shipping fee even though item marked FREE Prime delivery at checkout.", True),
    ("Disputing charge with bank because Amazon support refused to refund unauthorized order.", True),
    ("How long does it take for pending charge hold to fall off my checking account?", True),
    ("Gift card balance disappeared after placing order that was later cancelled.", True),
    ("My bank alerted me of suspicious transactions originating from Amazon Marketplace.", True),
    ("Invoice requested for tax filing does not show tax registration number.", True)
]

for idx, (text, esc) in enumerate(billing_cases, 134):
    golden_cases.append({
        "id": idx,
        "text": f"@AmazonHelp {text}",
...
# 6. GENERAL_FEEDBACK_COMPLAINT & EDGE CASES (34 cases)
feedback_cases = [
    ("Your delivery driver threw my box over the gate and broke my porch lights!", True),
    ...
    ("Your driver was speeding down our quiet residential street dangerously!", True),
    ...
    # Edge Cases: Sarcasm, Passive Aggression, Multi-Intent
    ("Thanks Amazon for delivering my package to the roof! Great job!", False), # Sarcasm
    ("Great job sending a size 5 shoe when I ordered 10! Truly amazing service.", False), # Sarcasm
    ("My package is late AND the item inside is broken AND you charged me twice!", True), # Multi-intent / Escalation
    ("I love waiting 45 minutes on hold listening to elevator music!", False), # Sarcasm
    ("Driver delivered my package directly into my swimming pool. Fantastic work.", True), # Property damage / Escalation
    ("Is customer support operated by actual humans or just broken robots?", False),
    ("Filing a report with the Better Business Bureau if no response by 5 PM.", True), # Legal / Regulatory threat
    ("Kudos to Amazon for turning a 2-day delivery into a 2-week adventure!", False), # Sarcasm
    ("I demand to speak to a manager or director immediately regarding driver trespassing.", True) # Escalation
]

for idx, (text, esc) in enumerate(feedback_cases, 167):
    golden_cases.append({
        "id": idx,
        "text": f"@AmazonHelp {text}",
...
out_path = os.path.join(data_dir, "golden_evaluation_set_200.json")

with open(out_path, "w", encoding="utf-8") as f:
    json.dump(golden_cases, f, indent=2)

print(f"Generated 200 Golden Evaluation Set examples at {out_path}")
